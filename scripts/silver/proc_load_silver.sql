...
BEGIN
	...
	BEGIN TRY
		...
		SELECT
			...
			END AS sls_due_dt,
			CASE
				WHEN sls_sales IS NULL OR sls_sales <=0 OR sls_sales != sls_sales_quantity * ABS(sls_price)
					THEN sls_sales_quantity * ABS(sls_price)
				ELSE sls_sales 
			END AS sls_sales, -- Recalculate sales if original value is missing or incorrect
			sls_sales_quantity,
			CASE 
				WHEN sls_price IS NULL OR sls_price <= 0
					THEN sls_sales / NULLIF(sls_sales_quantity, 0)
